...
- Maintainers do not need to run versioning or publish locally for the normal flow

The normal flow is: create a changeset in your feature PR, merge the work into `main`, and GitHub Actions handles versioning, publish, tags, and GitHub Releases automatically.
...
6. Once the changesets are on `main`, the release workflow runs `changeset version`, refreshes the lockfile, commits those version updates back to `main`, and publishes packages to npm
7. GitHub Actions creates any missing `@better-translate/*@version` git tags locally and pushes only the new tags back to `origin`
8. GitHub Actions creates or reuses matching GitHub Release entries for those new package tags with auto-generated release notes
...
- A valid `NPM_TOKEN`
- A repository secret named `RELEASE_DEPLOY_KEY` containing the private SSH key for a write-enabled deploy key
- Branch protection or rulesets that allow the release workflow to push its version bump commit back to `main`

You do not need a separate token for GitHub Releases. The workflow uses the built-in `GITHUB_TOKEN`.

For deploy-key based pushes on `main`:

1. Generate a dedicated SSH key pair for this repository
2. Add the public key in GitHub under `Settings` → `Deploy keys`
3. Enable `Allow write access`
4. If you use repository rulesets on `main`, add that deploy key to the bypass list with `Always allow`
5. Add the private key as the `RELEASE_DEPLOY_KEY` GitHub Actions secret
...
7. Confirm GitHub Actions is allowed to push the workflow-created version bump commit to `main`
