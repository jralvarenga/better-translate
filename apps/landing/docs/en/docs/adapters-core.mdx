...
`directions` is optional. Any locale you omit defaults to `"ltr"`.
...
Use the configured translator when you need locale direction metadata:

```tsx showLineNumbers
translator.getDirection({ locale: "es" }) // "rtl"
translator.isRtl({ locale: "es" }) // true

// Per-call rtl overrides do not change message lookup.
